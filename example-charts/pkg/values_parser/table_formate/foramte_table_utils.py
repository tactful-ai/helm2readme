...
def format_value_for_table(value, prefix="", key_to_comment_map=None):
    ...
    if isinstance(value, list):
        ...
        for index, item in enumerate(value):
            formatted_item = format_raw(item, f"{prefix}[{index}]", list_comments['afterComments'][index])
        ...
        formatted_entries.append(format_raw(value, prefix, dict_comments['beforeComments']))
